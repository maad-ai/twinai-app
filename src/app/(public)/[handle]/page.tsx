...
export default async function PublicTwinPage({
  ...
  return (
    <main className="max-w-md mx-auto px-5 py-10">
          ...
          <div className="flex items-center justify-center gap-2 mt-4">
            {socialEntries.map(([key, value]) => (
              <a
                ...
                className="w-9 h-9 rounded-full bg-white border border-black/[0.08] shadow-sm flex items-center justify-center text-[#475569] hover:text-[#A855F7] hover:border-[#A855F7]/30 transition-colors"
              >
      ...
      {/* Bio */}
      {bio && (
        <div className="card rounded-2xl p-5 mb-7">
          <p className="text-xs font-600 uppercase tracking-wider text-[#94A3B8] mb-2">About</p>
          <p className="text-[15px] text-[#475569] leading-relaxed whitespace-pre-line">{bio}</p>
        </div>
      )}
